...
test('agent can view reviews page with overall satisfaction and star breakdown', function () {
...
test('agent only sees their own reviews', function () {
    $this->actingAs($this->user);

    $otherAgent = Agent::factory()->create();
    $otherPackage = Package::factory()->create(['agent_id' => $otherAgent->id]);
    $otherRes = Reservation::factory()->completed()->create(['agent_id' => $otherAgent->id, 'bookable_type' => 'package', 'bookable_id' => $otherPackage->id, 'guest_name' => 'Stranger']);
    ...
    Review::factory()->create([
        ...
        'comment' => 'Review for other agent only',
    ]);

    Livewire::test('pages::reviews.index')
        ->assertDontSee('Review for other agent only');
});

test('agent can filter reviews by star rating', function () {
    $this->actingAs($this->user);

    $package = Package::factory()->create(['agent_id' => $this->agent->id]);

    $res1 = Reservation::factory()->completed()->create(['agent_id' => $this->agent->id, 'bookable_type' => 'package', 'bookable_id' => $package->id]);
    $res2 = Reservation::factory()->completed()->create(['agent_id' => $this->agent->id, 'bookable_type' => 'package', 'bookable_id' => $package->id]);
