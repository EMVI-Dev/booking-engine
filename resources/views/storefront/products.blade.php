...
<head>
    ...
    @php
        ...
        $share = $seo->homeShareImage($agent, [], $products);
    @endphp
    @include('storefront.partials.seo', [
        'agent' => $agent,
        'title' => __('Activities').' · '.$agent->name,
        'description' => __('See activities from :name. Prices are listed so you can pick one and reserve a spot online.', ['name' => $agent->name]),
        'url' => route('storefront.products'),
        'share' => $share,
        'schema' => $seo->productCatalogGraph($agent, $products),
    ])
        ...
        <div class="space-y-3">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <span class="self-start md:self-auto px-3 py-1 rounded-full text-xs font-bold bg-sky-50 text-sky-700 dark:bg-sky-950/80 dark:text-sky-300 border border-sky-200 dark:border-sky-800 shrink-0">
                    {{ __(':count Items Available', ['count' => $products->total()]) }}
                </span>
            </div>
        </div>
