...
<body x-data="{ mobileMenuOpen: false }" class="min-h-screen flex flex-col bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-slate-100 antialiased selection:bg-brand-600 selection:text-white">
    ...
    <header class="sticky top-0 z-40 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-zinc-800 transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
            <a href="{{ route('home') }}" class="flex items-center gap-2.5 min-w-0 group">
                @if ($agent->logo_path)
                    <img src="{{ Storage::url($agent->logo_path) }}" alt="{{ $agent->name }}" class="h-9 w-9 rounded-xl object-cover border border-slate-200/80 dark:border-zinc-800 shadow-xs shrink-0 group-hover:scale-105 transition-transform bg-white dark:bg-zinc-800" />
                @else
                ...
                @endif
                <span class="truncate max-w-[160px] sm:max-w-xs font-black text-xs sm:text-sm text-slate-900 dark:text-white group-hover:text-brand-600 transition">{{ $agent->name }}</span>
            </a>

            <!-- Desktop Nav -->
            <nav class="hidden md:flex items-center gap-3 text-xs font-bold">
                <a href="{{ route('home') }}" class="text-slate-600 dark:text-slate-400 hover:text-brand-600 transition">
                    {{ __('Home') }}
                </a>
                <a href="{{ route('storefront.packages') }}" class="text-slate-600 dark:text-slate-400 hover:text-brand-600 transition">
                    {{ __('Packages') }}
                </a>
                <span class="text-brand-600 dark:text-white px-2.5 py-1 rounded-lg bg-brand-50 dark:bg-brand-950/80">
                    {{ __('Activities & Rentals') }}
                </span>
                <a href="{{ route('storefront.terms') }}" class="text-slate-600 dark:text-slate-400 hover:text-brand-600 transition">
                    {{ __('Terms') }}
                </a>
            </nav>
        ...
        >
            ...
            <a href="{{ route('storefront.products') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold text-brand-600 dark:text-white bg-brand-50/70 dark:bg-brand-950/50 transition">
                <i class="fa-solid fa-box-open w-4 text-brand-500"></i>
                <span>{{ __('Activities & Rentals') }}</span>
            </a>
            <a href="{{ route('storefront.terms') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-zinc-800 hover:text-brand-600 transition">
                ...
                @foreach ($products as $prod)
                    <div class="group flex flex-col justify-between rounded-2xl sm:rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs hover:shadow-xl hover:border-brand-400 dark:hover:border-brand-600 transition-all duration-200 overflow-hidden">
                        ...
                        <div class="relative aspect-video w-full overflow-hidden bg-gradient-to-br from-slate-900 via-sky-950 to-slate-900 shrink-0">
                            @if ($prod->cover_photo)
                                <img src="{{ Storage::url($prod->cover_photo) }}" alt="{{ $prod->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                            @else
    ...
    @if ($agent->contact_whatsapp)
        ...
        <div class="hidden lg:block fixed bottom-6 right-6 z-50">
            <a href="https://wa.me/{{ $waNumber }}?text={{ urlencode('Hello ' . $agent->name . ', I am browsing your services & equipment rentals.') }}"
                target="_blank"
