...
<div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    {{ \Illuminate\Support\Carbon::parse($manifestDate)->format('l, F d, Y') }} &bull; {{ $this->manifestReservations->sum('pax_count') }} {{ __('Total Passengers') }} ({{ $this->manifestReservations->count() }} {{ __('Bookings') }})
                </p>
            </div>
        </div>
    ...
    <div class="rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs overflow-hidden print:border-none print:shadow-none print:p-0">
        <!-- Document Header -->
        <div class="p-6 border-b border-slate-200/80 dark:border-zinc-800 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-800/30 print:bg-white print:border-b-2 print:border-slate-800 print:px-0 print:py-3">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-xs font-black tracking-wider uppercase text-indigo-600 dark:text-indigo-400 print:text-slate-800">
                ...
                <p class="text-xs text-slate-500 print:text-slate-600 mt-0.5">
                    {{ __('Generated:') }} {{ now()->format('d M Y, H:i') }} &bull; {{ $this->manifestReservations->count() }} {{ __('Total Bookings') }}
                </p>
            </div>
            <div class="text-right">
                <div class="inline-flex flex-col items-center justify-center px-4 py-2 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200/60 dark:border-indigo-800/60 print:border-2 print:border-slate-800 print:bg-slate-100">
                    <span class="text-2xl sm:text-3xl font-black text-indigo-600 dark:text-indigo-400 print:text-slate-900 leading-none">
                        {{ $this->manifestReservations->sum('pax_count') }}
                    </span>
                    <span class="text-[10px] font-extrabold uppercase tracking-wider text-indigo-700 dark:text-indigo-300 print:text-slate-700 mt-1">
                        {{ __('Total Pax') }}
                    </span>
                </div>
            </div>
        ...
        <div class="overflow-x-auto print:overflow-visible">
            <table class="w-full text-left text-xs border-collapse min-w-[700px] print:min-w-full manifest-print-table">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-zinc-800 bg-slate-100/70 dark:bg-zinc-800/60 text-slate-600 dark:text-zinc-400">
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px] w-10 text-center">#</th>
                        <!-- Print-only Checkbox Box for Clipboard Check-In -->
                        <th class="hidden print:table-cell py-3 px-2 font-black uppercase tracking-wider text-[9px] w-12 text-center">{{ __('Check') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px]">{{ __('Booking Code') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px]">{{ __('Lead Guest & Contact') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px]">{{ __('Tour / Experience') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px] text-center w-14">{{ __('Pax') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px]">{{ __('Notes / Pickup Location') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px] text-center w-24">{{ __('Status') }}</th>
                        <th class="py-3 px-3 font-black uppercase tracking-wider text-[10px] text-center print:hidden w-20">{{ __('WhatsApp') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                    @forelse ($this->manifestReservations as $idx => $res)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-zinc-800/30 transition">
                            <td class="py-3 px-3 text-center font-mono font-bold text-slate-400 print:text-slate-900">{{ $idx + 1 }}</td>
                            
                            <!-- Print-only Checkbox for manual pen tick -->
                            <td class="hidden print:table-cell py-3 px-2 text-center">
                                <div class="w-4 h-4 mx-auto border border-slate-400 rounded-xs"></div>
                            </td>

                            <td class="py-3 px-3 font-mono font-extrabold text-indigo-600 dark:text-indigo-400 print:text-slate-900">
                                #{{ $res->code }}
                            </td>
                            <td class="py-3 px-3">
                                <p class="font-extrabold text-slate-900 dark:text-white print:text-slate-900 leading-tight">{{ $res->guest_name }}</p>
                                <p class="text-[11px] text-slate-500 print:text-slate-700 font-mono mt-0.5">{{ $res->guest_contact ?: '—' }}</p>
                            </td>
                            <td class="py-3 px-3">
                            ...
                            <td class="py-3 px-3 text-center font-black text-sm text-slate-900 dark:text-white print:text-slate-900">
                                {{ $res->pax_count }}
                            </td>
                            <td class="py-3 px-3 text-slate-600 dark:text-slate-300 print:text-slate-800 text-[11px] max-w-xs">
                                {{ $res->notes ?: __('—') }}
                            </td>
                            <td class="py-3 px-3 text-center">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase print:border print:border-slate-400 print:text-slate-900 print:bg-white
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="py-12 text-center text-slate-400 print:text-slate-600">
                                <i class="fa-solid fa-clipboard-check text-3xl mb-2 text-slate-300 dark:text-zinc-700 print:hidden block"></i>
        ...
        <div class="hidden print:grid grid-cols-3 gap-6 pt-8 mt-6 border-t border-slate-300 text-xs text-slate-700">
            <div class="space-y-6">
                <p class="font-bold uppercase text-[10px] tracking-wider text-slate-500">{{ __('Tour Guide / Lead Staff') }}</p>
                <div class="border-b border-slate-400 pt-6"></div>
                <p class="text-[10px] text-slate-400">{{ __('Name & Signature') }}</p>
            </div>
            <div class="space-y-6">
                <p class="font-bold uppercase text-[10px] tracking-wider text-slate-500">{{ __('Driver / Dispatch Check') }}</p>
                <div class="border-b border-slate-400 pt-6"></div>
                <p class="text-[10px] text-slate-400">{{ __('Name & Vehicle / Unit ID') }}</p>
            </div>
            <div class="space-y-6">
                <p class="font-bold uppercase text-[10px] tracking-wider text-slate-500">{{ __('Departure Verified') }}</p>
                <div class="border-b border-slate-400 pt-6"></div>
