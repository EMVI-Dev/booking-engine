    ...
    <div class="rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse min-w-[760px]">
                <thead>
                    <tr class="border-b border-slate-200/80 dark:border-zinc-800 bg-slate-50/70 dark:bg-zinc-800/40 text-slate-500 dark:text-slate-400">
                        ...
                        @foreach ($this->timelineDays as $d)
                            <th class="py-3.5 px-3 text-center font-bold text-xs {{ $d['isToday'] ? 'bg-indigo-50/70 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400' : '' }}">
                                <span class="block text-[10px] uppercase font-extrabold opacity-75">{{ $d['dayName'] }}</span>
                        ...
                        <tr class="hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition">
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <span class="text-[10px] text-slate-400 block mt-0.5">
                                            {{ __('Cap:') }} {{ $product->capacity_per_day ?: 20 }} {{ __('pax/day') }}
                                        </span>
                                    </div>
                                </div>
                            </td>
                            ...
                            @foreach ($row['days'] as $cell)
                                <td class="py-3 px-2 text-center align-middle">
                                    @if ($cell['isBlocked'])
                                        <div class="p-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200/60 dark:border-rose-900/40 text-rose-700 dark:text-rose-300 text-[10px] font-bold">
                                            <i class="fa-solid fa-lock text-[9px] mr-1"></i>{{ __('Blocked') }}
                                        </div>
                                    @elseif ($cell['totalPax'] > 0)
                                        <div class="p-2 rounded-xl {{ $cell['occupancyRate'] >= 90 ? 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300' : ($cell['occupancyRate'] >= 50 ? 'bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300' : 'bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-300') }} text-center space-y-1 shadow-2xs">
                                            ...
                                            <div class="w-full bg-black/10 dark:bg-white/10 h-1.5 rounded-full overflow-hidden">
                                                <div
                                                    class="h-full rounded-full {{ $cell['occupancyRate'] >= 90 ? 'bg-rose-600' : ($cell['occupancyRate'] >= 50 ? 'bg-amber-500' : 'bg-indigo-600') }}"
                                                    style="width: {{ $cell['occupancyRate'] }}%;"
                                                ></div>
                                            </div>
                                    ...
                                    @else
                                        <div class="p-2 rounded-xl bg-slate-50 dark:bg-zinc-800/30 text-slate-300 dark:text-zinc-600 text-[10px] font-medium border border-dashed border-slate-200 dark:border-zinc-800">
                                            {{ __('Open') }}
                                        </div>
                                    @endif
                                </td>
