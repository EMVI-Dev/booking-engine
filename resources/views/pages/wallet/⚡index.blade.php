...
new #[Title('Wallet & Payouts')] class extends Component {
    ...
    public function metrics(): array
    {
        ...
        $totalFees = (float) $agent->walletTransactions()
    ...
    public function transactions(): LengthAwarePaginator
    {
        ...
        return $agent->walletTransactions()
                ...
                $query->where(function ($q) {
                    $q->where('description', 'like', "%{$this->search}%")
                        ->orWhereHas('reservation', function ($resQuery) {
                            $resQuery->where('code', 'like', "%{$this->search}%")
...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    @else
        <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-900/70 border border-slate-200/80 dark:border-zinc-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
            <div class="flex items-center gap-3">
                <span class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-950/70 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs shrink-0">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="relative overflow-hidden p-5 rounded-3xl bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 text-white border border-indigo-800/50 shadow-md flex flex-col justify-between space-y-4">
        ...
        <div class="p-5 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs flex flex-col justify-between space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-900">
            ...
            <div class="pt-2 border-t border-slate-100 dark:border-zinc-800 text-[11px] text-slate-400 flex items-center justify-between">
        ...
        <div class="p-5 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs flex flex-col justify-between space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="p-1 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 text-xs">
            ...
            <div class="pt-2 border-t border-slate-100 dark:border-zinc-800 text-[11px] text-slate-400 flex items-center justify-between">
                ...
                <span class="font-bold text-slate-600 dark:text-slate-300">Rp {{ number_format($this->metrics['total_fees'], 0, ',', '.') }}</span>
            </div>
        </div>
    ...
    <div class="p-4 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-zinc-800">
            <!-- Tabs Switcher -->
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
                ...
                <div class="w-full sm:w-52">
                    ...
                    @else
                        <x-select
                            wire:key="wallet-filter-payouts"
                            wire:model.live="statusFilter"
                            :options="[
                                'all' => __('All Statuses'),
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @forelse ($this->transactions as $trx)
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition-colors">
                                ...
                                <td class="py-3 px-3">
                                    <div class="flex items-center gap-2.5 min-w-[200px]">
                                        @if ($trx->type->value === 'booking_earning')
                                            <span class="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs shrink-0">
                                                <i class="fa-solid fa-arrow-down-left"></i>
                                            </span>
                                        @elseif ($trx->type->value === 'payout_withdrawal')
                                            <span class="w-7 h-7 rounded-lg bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs shrink-0">
                                                <i class="fa-solid fa-arrow-up-right"></i>
                                            </span>
                                        @else
                                            <span class="w-7 h-7 rounded-lg bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 flex items-center justify-center text-xs shrink-0">
                                ...
                                <td class="py-3 px-3 whitespace-nowrap font-bold {{ $trx->net_amount >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white' }}">
                                    {{ $trx->net_amount >= 0 ? '+' : '' }}Rp {{ number_format((float) $trx->net_amount, 0, ',', '.') }}
                                </td>
                                ...
                                <td class="py-3 px-3 whitespace-nowrap">
                                    ...
                                    @elseif ($trx->status->value === 'pending_escrow')
                                        <div class="flex flex-col">
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 w-fit">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-400">
                                            {{ __('Cancelled') }}
                                        </span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-slate-400">
                                    ...
                                    <p class="font-bold text-sm text-slate-600 dark:text-slate-300">{{ __('No ledger transactions yet') }}</p>
                                    <p class="text-xs text-slate-400 mt-1">{{ __('Paid guest reservations on your storefront will automatically appear here.') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @forelse ($this->payoutRequests as $payout)
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition-colors">
                                <td class="py-3 px-3 whitespace-nowrap font-mono font-bold text-slate-900 dark:text-white">
                                ...
                                <td class="py-3 px-3 whitespace-nowrap">
                                    ...
                                    @elseif ($payout->status->value === 'processing')
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                                            <i class="fa-solid fa-spinner fa-spin text-[10px]"></i>
                                            {{ __('Processing Transfer') }}
                                        </span>
                                    @elseif ($payout->status->value === 'rejected')
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800" title="{{ $payout->rejection_reason }}">
                                ...
                                <td class="py-3 px-3 text-slate-500 dark:text-slate-400">
                                    ...
                                    @elseif ($payout->rejection_reason)
                                        <span class="text-[11px] text-rose-500 font-semibold truncate block max-w-[200px]" title="{{ $payout->rejection_reason }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-slate-400">
                                    ...
                                    <p class="font-bold text-sm text-slate-600 dark:text-slate-300">{{ __('No payout requests submitted yet') }}</p>
                                    <p class="text-xs text-slate-400 mt-1">{{ __('When you have available balance, click "Request Payout" to initiate a bank transfer.') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @if ($showPayoutModal)
        @teleport('body')
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                <div class="w-full max-w-lg rounded-3xl bg-white dark:bg-zinc-900 shadow-2xl border border-slate-200/80 dark:border-zinc-800 flex flex-col my-8">
                    <!-- Modal Header -->
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200/80 dark:border-zinc-800 space-y-1.5">
                            <span class="text-[10px] uppercase font-bold tracking-wider text-slate-400">{{ __('Destination Bank Account') }}</span>
                            <div class="flex items-center justify-between">
                                <div>
                        ...
                        <form wire:submit="submitPayoutRequest" class="space-y-4">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1.5">
                                    ...
                                    <span class="text-xs text-slate-500 dark:text-slate-400">
                                        {{ __('Available:') }} <strong class="text-slate-900 dark:text-white">Rp {{ number_format($this->metrics['available'], 0, ',', '.') }}</strong>
                                    </span>
                                </div>

                                <div class="relative">
                                    <span class="absolute left-3.5 top-1/2 -translate-y-1/2 font-black text-xs text-slate-400">Rp</span>
                                    <x-input id="payoutAmount" type="number" step="1000" min="50000" wire:model="payoutAmount"
                                        placeholder="500000"
                                ...
                                <div class="flex items-center gap-1.5 mt-2">
                                    <span class="text-[10px] uppercase font-bold text-slate-400 mr-1">{{ __('Quick Fill:') }}</span>
                                    <button type="button" wire:click="quickFillAmount(25)" class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-700 dark:text-slate-300 transition cursor-pointer">
                                        25%
                                    </button>
                            ...
                            <div>
                                ...
                                <x-textarea id="payoutNotes" wire:model="payoutNotes" rows="2" placeholder="e.g. Monthly crew operations disbursement..." />
                            </div>

                            <!-- Actions -->
                            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-zinc-800">
                                <x-button type="button" variant="secondary" wire:click="$set('showPayoutModal', false)" class="text-xs font-bold">
                                    {{ __('Cancel') }}
                                </x-button>
