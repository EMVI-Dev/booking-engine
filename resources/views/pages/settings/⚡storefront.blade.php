    ...
    <form wire:submit="updateStorefrontSettings" class="w-full space-y-6">
        ...
        <div class="p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
            ...
            <div class="space-y-3">
                ...
                <div class="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200 dark:border-zinc-800">
                    <x-checkbox
                ...
                <div class="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200 dark:border-zinc-800">
                    <x-checkbox
                        id="show_reviews"
                        wire:model="show_reviews"
                        :label="__('Display Verified Guest Reviews on Storefront')"
                        :description="__('Showcase authentic guest ratings, comments, and experience feedback on your public landing page.')"
                    />
                </div>
                ...
                <div class="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200 dark:border-zinc-800">
                    <x-checkbox
                        id="show_inclusions_preview"
                        wire:model="show_inclusions_preview"
                        :label="__('Show Package Inclusions Preview Chips')"
                        :description="__('Display included product badges directly on package listing cards in catalog view.')"
                    />
                </div>
            </div>
        </div>
        ...
        <div class="p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <span class="w-9 h-9 rounded-xl bg-amber-100 dark:bg-amber-950/80 text-amber-600 dark:text-amber-400 flex items-center justify-center text-sm shrink-0">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] font-bold uppercase text-emerald-600 dark:text-emerald-400 tracking-wider">
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <span class="w-9 h-9 rounded-xl bg-indigo-100 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-sm shrink-0">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] font-bold uppercase text-indigo-600 dark:text-indigo-400 tracking-wider">
                                    {{ __('Operator Review') }}
                                </span>
                            </div>
                        </div>
        ...
        <div class="p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-input id="hero_headline" wire:model="hero_headline" type="text" placeholder="e.g. Unforgettable Bali Expeditions" :error="$errors->has('hero_headline')" />
                    <p class="text-[11px] text-slate-500 mt-1">{{ __('Leave blank to use your business name automatically.') }}</p>
                    <x-input-error :messages="$errors->get('hero_headline')" />
                </div>

                <div>
                    <x-label for="hero_tagline" :value="__('Custom Hero Tagline (Optional)')" />
                    <x-input id="hero_tagline" wire:model="hero_tagline" type="text" placeholder="e.g. Direct bookings & guaranteed private tours" :error="$errors->has('hero_tagline')" />
        ...
        <div class="p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
            <div class="flex items-center gap-2.5 pb-2 border-b border-slate-100 dark:border-zinc-800">
                <span class="p-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/70 text-amber-600 dark:text-amber-400 text-xs">
            ...
            <div class="space-y-2">
                ...
                <x-textarea id="terms_and_conditions" wire:model="terms_and_conditions" rows="5" placeholder="Detail your standard policies: cancellation cutoff rules, health and physical requirements, weather rescheduling policies, and guest liability disclaimers..." :error="$errors->has('terms_and_conditions')" />
                <p class="text-[11px] text-slate-500">{{ __('These terms are frozen into the guest reservation snapshot upon booking.') }}</p>
        ...
        <div class="flex items-center gap-4 pt-2">
            ...
            <div x-data="{ shown: false, timeout: null }"
                 x-init="@this.on('storefront-updated', () => { clearTimeout(timeout); shown = true; timeout = setTimeout(() => { shown = false }, 2500); })"
