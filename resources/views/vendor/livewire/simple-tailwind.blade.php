...
$scrollIntoViewJsSnippet = ($scrollTo !== false)
    ? <<<JS
       (\$el.closest('{$scrollTo}') || document.querySelector('{$scrollTo}')).scrollIntoView({ behavior: 'smooth', block: 'nearest' })
    JS
    : '';
@endphp
...
<div>
    @if ($paginator->hasPages())
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 py-3 select-none transition-all duration-200">
            ...
            <div class="flex items-center gap-1.5 transition-opacity duration-200" wire:loading.class="opacity-70">
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_string($element))
                        <span class="px-2 py-1 text-xs font-bold text-slate-400 dark:text-slate-500">{{ $element }}</span>
                    @endif
