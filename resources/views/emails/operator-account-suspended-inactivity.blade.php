...
<head>
    ...
    <title>{{ __('Account Suspended - Inactivity Notice') }}</title>
</head>
                    ...
                    <tr>
                        <td style="padding: 28px 28px 16px 28px;">
                            ...
                            <p style="margin: 0 0 16px 0; font-size: 14px; color: #475569; line-height: 1.6;">
                                {{ __('Your operator account for **:operator** has been suspended because there has been no activity recorded for over :days days (3 months).', ['operator' => $operator->name, 'days' => $inactiveDays]) }}
                            </p>
                            <p style="margin: 0 0 20px 0; font-size: 14px; color: #475569; line-height: 1.6;">
                                {{ __('While suspended, your storefront is paused and cannot receive new online reservations. All your existing data, settings, products, and past bookings remain completely safe in our database.', ['app' => config('app.name', 'TravelEngine')]) }}
                            </p>
                            ...
                            <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; padding: 18px; margin-bottom: 24px;">
                                ...
                                <tr>
                                    <td style="padding: 6px 0; font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase;">{{ __('Status') }}</td>
                                    <td align="right" style="padding: 6px 0; font-size: 12px; font-weight: 700; color: #e11d48;">{{ __('Suspended (Inactive)') }}</td>
                                </tr>
                                <tr>
                                    <td style="padding: 6px 0; font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase;">{{ __('Inactivity Duration') }}</td>
                                    <td align="right" style="padding: 6px 0; font-size: 12px; font-weight: 700; color: #475569;">{{ $inactiveDays }} {{ __('days') }}</td>
                                </tr>
                            </table>

                            <!-- Instructions to Reactivate -->
                            <div style="background-color: #fffbeb; border: 1px solid #fef3c7; border-radius: 16px; padding: 18px; margin-bottom: 20px;">
                                <div style="font-size: 13px; font-weight: 800; color: #92400e; margin-bottom: 6px;">
                                    {{ __('How to Reactivate Your Account') }}
                                </div>
                                <p style="margin: 0; font-size: 13px; color: #78350f; line-height: 1.5;">
                                    {{ __('To reopen your storefront and resume taking bookings, simply reach out to our platform support team. We will verify your account and reactivate your portal immediately.') }}
                                </p>
                            </div>

                            <!-- CTA Button -->
                            <div style="margin-bottom: 24px; text-align: center;">
                                <a href="mailto:{{ $supportEmail }}?subject=Reactivate%20Operator%20Account%20-%20{{ urlencode($operator->name) }}" style="display: inline-block; padding: 14px 32px; background-color: {{ $platformColor }}; color: {{ $platformInk }}; text-decoration: none; border-radius: 14px; font-weight: 800; font-size: 14px; box-shadow: 0 4px 6px -1px rgba(16, 23, 48, 0.12);">
                                    {{ __('Contact Support to Reactivate') }} &rarr;
                                </a>
                            </div>

                            <p style="margin: 0 0 16px 0; font-size: 13px; color: #64748b;">
                                {{ __('Platform Support: :email', ['email' => $supportEmail]) }}
                            </p>
                        </td>
                    </tr>
