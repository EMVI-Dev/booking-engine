@php
    ...
    $waGuestUrl = $reservation->guest_contact
        ? $waService->buildWhatsAppUrl($reservation->guest_contact, "Hello {$reservation->guest_name}, this is {$operator->name} regarding your booking #{$code} for {$bookableTitle}.")
        : null;

    $split = $payment?->split_details ?? [];
    $operatorNet = $split['operator_amount'] ?? ($split['agent_amount'] ?? ($payment ? round($payment->amount * 0.9, 2) : 0));
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
                            ...
                            <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; padding: 18px;">
                                ...
                                <tr>
                                    ...
                                    <td align="right" style="padding: 4px 0; font-size: 15px; font-weight: 900; color: #059669;">
                                        Rp {{ number_format((float) $operatorNet, 0, ',', '.') }}
                                    </td>
                                </tr>
                            </table>
                        </td>
