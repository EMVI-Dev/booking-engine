...
$alignmentClasses = match ($alignment) {
    ...
    'bottom-start', 'bottom-left' => 'origin-top-left start-0 mt-2',
    'bottom-end', 'bottom-right', 'right' => 'origin-top-right end-0 mt-2',
...
$containerClasses = $width === 'full' ? 'relative w-full' : 'relative inline-block text-left';
@endphp

<div {{ $attributes->merge(['class' => $containerClasses]) }} x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
    <div @click="open = ! open">
        {{ $trigger }}
    </div>
