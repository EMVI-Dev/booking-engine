...
@php
    ...
    $baseClasses =
        'h-10 w-full rounded-xl border text-xs sm:text-sm shadow-xs transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-offset-0 disabled:bg-slate-100 dark:disabled:bg-zinc-800 disabled:cursor-not-allowed';
    $stateClasses = $error
...
<div x-data="{
    ...
    minDate: '{{ $minDate }}',
    maxDate: '{{ $maxDate }}',
    blackoutDates: @js($blackoutDates ?? []),
    monthNames: [
    ...
    getBlackoutReason(dateStr) {
        if (!dateStr || !this.blackoutDates) return null;
        if (Array.isArray(this.blackoutDates)) {
            return this.blackoutDates.includes(dateStr) ? '{{ __('Blackout Date') }}' : null;
        }
        return this.blackoutDates[dateStr] || null;
    },
    get daysInMonth() {
        ...
        for (let i = 0; i < firstDayIndex; i++) {
            days.push({ day: null, dateStr: null, isCurrentMonth: false, disabled: true, isBlackout: false, blackoutReason: null });
        }
        ...
        for (let i = 1; i <= totalDays; i++) {
            ...
            const blackoutReason = this.getBlackoutReason(dateStr);
            const isBlackout = !!blackoutReason;

            let isDisabled = isBlackout;
    ...
    setPreset(daysFromNow) {
        ...
        if ((!this.minDate || dateStr >= this.minDate) && !this.getBlackoutReason(dateStr)) {
    ...
    <button type="button" x-on:click="togglePicker()"
        ...
        <div class="flex items-center gap-2.5 truncate">
            ...
            <span class="truncate"
                ...
                x-text="formattedLabel"></span>
        </div>
        <i class="fa-solid fa-chevron-down text-[11px] text-slate-400 transition-transform duration-200 shrink-0"
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
        ...
        @if ($presets)
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 text-[10px]">
                <button type="button" x-on:click="setPreset(0)"
                ...
                <button type="button" x-on:click="setPreset(1)"
                    class="px-2 py-1 rounded-lg bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-700 dark:text-slate-300 font-bold transition cursor-pointer shrink-0">
                    {{ __('Tomorrow') }}
                </button>
                <button type="button" x-on:click="setPreset(2)"
            ...
            <template x-for="(cell, index) in daysInMonth" :key="index">
                <div>
                    <template x-if="cell.day">
                        <button type="button" x-on:click="if (!cell.disabled) selectDate(cell.dateStr)"
                            :disabled="cell.disabled"
                            :title="cell.isBlackout ? (cell.blackoutReason || '{{ __('Blackout Date: Unavailable') }}') : ''"
                            class="w-full h-8 rounded-xl font-bold text-xs flex items-center justify-center transition-all cursor-pointer relative"
                            :class="{
                                'bg-indigo-600 text-white shadow-xs scale-105': cell.isSelected,
                                'bg-rose-50 dark:bg-rose-950/40 text-rose-500 dark:text-rose-400 line-through cursor-not-allowed border border-rose-200/60 dark:border-rose-900/50': cell.isBlackout && !cell.isSelected,
                                'text-slate-300 dark:text-zinc-700 cursor-not-allowed opacity-40': cell.disabled && !cell.isBlackout,
                                'text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-zinc-800': !cell.disabled && !cell.isSelected,
                                'border border-indigo-400 dark:border-indigo-600': cell.isToday && !cell.isSelected && !cell.isBlackout
                            }">
                            <span x-text="cell.day"></span>
                            <template x-if="cell.isBlackout">
                                <span class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-zinc-900"></span>
                            </template>
                        </button>
                    </template>
                </div>
        ...
        <!-- Blackout Legend (Rendered when blackout dates exist) -->
        <template x-if="Object.keys(blackoutDates).length > 0 || (Array.isArray(blackoutDates) && blackoutDates.length > 0)">
            <div class="pt-2 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-between text-[10px] text-slate-400 dark:text-zinc-500">
                <span class="inline-flex items-center gap-1.5 font-medium text-rose-600 dark:text-rose-400">
                    <span class="w-2 h-2 rounded-full bg-rose-500"></span>
                    <span>{{ __('Blackout Date') }}</span>
                </span>
                <span>{{ __('Unavailable for booking') }}</span>
            </div>
        </template>
    </div>
</div>
